...
{
    msgs2s::MsgGT2GSAddUser* msg = static_cast<msgs2s::MsgGT2GSAddUser*>(p);
    if (msg)
    {
...
{
    msgs2s::MsgGT2GSRemoveUser* msg = static_cast<msgs2s::MsgGT2GSRemoveUser*>(p);
    if (msg)
    {
...
{
    msgs2s::MsgGT2GSOfflineUser* msg = static_cast<msgs2s::MsgGT2GSOfflineUser*>(p);
    if (msg)
    {
...
void GateSession::parseGateRegister(google::protobuf::Message* p, pb_flag_type flag)
{
	...
	if (msg)
	{
        ...
        msgs2s::MsgServerRegister m ; 
        m.set_serverid(gGameServer.getServerID());
        msgs2s::ServerVersion* p2 = m.mutable_version();
...
void GateSession::initPBModule()
{
	...
	ProtocMsgBase<GateSession>::registerCBFun(PROTOCO_NAME(msgs2s::MsgServerRegister),      &GateSession::parseGateRegister);
    ProtocMsgBase<GateSession>::registerCBFun(PROTOCO_NAME(msgs2s::MsgGT2GSAddUser),        &GateSession::parseAddUser);
    ProtocMsgBase<GateSession>::registerCBFun(PROTOCO_NAME(msgs2s::MsgGT2GSRemoveUser),     &GateSession::parseRemoveUser);
    ProtocMsgBase<GateSession>::registerCBFun(PROTOCO_NAME(msgs2s::MsgGT2GSOfflineUser),     &GateSession::parseOffline);
}
