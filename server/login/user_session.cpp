...
void UserLoginSession::initPBModule()
{
	...
	ProtocMsgBase<UserLoginSession>::registerCBFun(PROTOCO_NAME(clientmsg::LoginRequest),  &UserLoginSession::parseLoginGame);
}
void UserLoginSession::parseLoginGame(google::protobuf::Message* p, pb_flag_type flag)
{
	clientmsg::LoginRequest* msg = static_cast<clientmsg::LoginRequest*>(p);
	if (msg)
	{
		static int login_account = 0;
		login_account ++;
		setState(_checking_data_);
        //Mylog::log_player( LOG_INFO, "Handle Account[%s] Login [%d]!", msg->name().c_str(), login_account);
		gLGAccountMgr.checkAccount(msg->name(), msg->pwd(), this);
//         clientmsg::CharMove m;
//         m.set_charid(1);
//         m.set_dir(10.f);
//         m.set_x(10.f);
//         m.set_x(10.f);
