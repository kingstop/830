...
void UserSession::initPBModule()
{
	...
	ProtocMsgBase<UserSession>::registerCBFun(PROTOCO_NAME(clientmsg::LoginGame),  &UserSession::parseLoginGame);
    ProtocMsgBase<UserSession>::registerCBFun(PROTOCO_NAME(clientmsg::ClientExit),  &UserSession::parseExitGame);
}
...
{
	clientmsg::LoginGame* msg = static_cast<clientmsg::LoginGame*>(p);
	if (msg)
	{
